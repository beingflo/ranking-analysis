...
use skillratings::{
    trueskill::{expected_score_two_teams, trueskill_two_teams, TrueSkillConfig, TrueSkillRating},
    Outcomes,
};
use std::collections::HashMap;

pub type TrueSkillRanking = HashMap<String, TrueSkillRating>;

pub fn compute_trueskill_accuracy(matches: &Vec<Match>) -> f32 {
    ...
    for p in get_players(&matches) {
        ranking.insert(p.clone(), TrueSkillRating::new());
    }
    ...
    for (idx, m) in matches.iter().enumerate() {
        ...
        let (new_a, new_b) = trueskill_two_teams(
            &[
                ranking.get(&m.team_a[0]).unwrap().clone(),
                ranking.get(&m.team_a[1]).unwrap().clone(),
            ],
